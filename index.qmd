---
title: "OZFS, ZoneR, and zonepy"
subtitle: "Enabling cross-jurisdictional zoning analysis"
authors: 
...
abstract: >
  The development of software to conduct detailed analysis of the consequences of zoning regulations has historically been hampered by the lack of standardized, machine-readable data to encode those regulations. In this paper, we introduce an R package (zoneR) and a Python package (zonepy) that rely on a novel data standard (Open Zoning Feed Specification or OZFS) to identify all parcels across multiple jurisdiction where a user-specified prototype building would be allowed by right. We describe the Open Zoning Feed Specification (OZFS) standard that this software relies on and introduce a sample OZFS dataset that includes 11 municipalities in the Dallas-Fort Worth region of Texas. We demonstrate how zoneR can be used to determine which parcels from a set of 100,000 parcels can accommodate each of two different four-unit residential buildings, accounting the zoning regulations of their respective districts.
...
Inside individual proptech firms, bespoke JSON or GraphQL schemas presumably power automated tools and linked formulas. None, however, has been published as a public, versioned specification that cities, researchers, and software developers can adopt without reinvention.
